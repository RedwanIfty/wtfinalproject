function formValidation(search) {
	...
	if (Ticket_no=== "") {
		document.getElementById("error1").innerHTML="*Select Ticket Number JS";
		return false;
	}
	else{
	...
	if (date=== "") {
		document.getElementById("error2").innerHTML="*Select Date JS";
		return false;
	}
	else{
...
function myFunction() {
    ...
    x.style.display = "none";
	  ...
	  xhttp.open("POST", "../Controller/cancel_BookingAction.php", true);
	  xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
	  xhttp.send("Ticket_no="+Ticket_no+"&date="+date);
	}
}
